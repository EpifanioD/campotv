...
require_once('C:/wamp64/www/campotv/app/Model/Connection.php');
...
class NewsController {
    ...
    public function create()
    {
        try {
            ...
            $stmt = $this->connection->conn()->prepare("INSERT INTO news (title, description, image) VALUES(:TITLE, :DESCRIPTION, :IMAGE)");
    ...
    public function update()
    {
        try {
            ...
            $stmt = $this->connection->conn()->prepare("UPDATE news SET title = :TITLE, description = :DESCRIPTION, image = :IMAGE WHERE id = :ID");
    ...
    public function delete()
    {
        try {
            ...
            $stmt = $this->connection->conn()->prepare("DELETE FROM news WHERE id = :ID");

            $this->connection->conn()->beginTransaction();
            ...
            $this->connection->conn()->commit();
        } catch (\Exception $e) {

            $this->result['status'] = false;
            $this->connection->conn()->rollBack();
        }
    ...
    public function export( $method, $withImage ) {
        ...
        $stmt = $this->connection->conn()->prepare( "SELECT title, description, image, created_at FROM news ORDER BY created_at DESC LIMIT 3" );
        $stmt->execute();
        ...
        if( $method == 'exportXml' ) {
            ...
            foreach( $data as $key => $post ) {
                ...
                foreach ( $post as $attr => $val ) {
                    ...
                    if( $attr == 'image' )
                        $postXml->setAttribute('imagem', (($withImage == 1 ) ? $val : '') );
                    if( $attr == 'created_at' )
